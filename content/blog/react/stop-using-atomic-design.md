...
`ListItem`의 Props가 아예 다르게 정의되었고 (매우 줄어들었다) 하는 역할이라곤 각 `left`, `bottom`, `right` props 그리고 `children`로 전달되는 컴포넌트들(또는 텍스트)을 배치하는 역할만 하게 되었다. 레이아웃 또한 스타일에 불과하니 ListItem은 아직 원자일까, 레이아웃을 담당하니 템플릿일까.
