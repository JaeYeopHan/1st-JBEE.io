...
필요한 모든 webpack plugin과 loader를 설정해주며 lint rule도 잡아준다. TypeScript까지 정식으로 지원하니 사용하지 않을 이유가 없다. 한 가지 추가해줬던 것은 prettier 관련 설정과 husky 그리고 phase 관련 설정이었다.
...
이번 프로젝트에서는 사용하지 않았지만 craco 자체에서 여러 craco plugin들을 제공한다.
...
프로젝트 초기에 API 응답 포맷이 변경되거나 요구사항이 변경되는 경우가 많았는데, TypeScript의 Type Definition을 수정해준 뒤, compile error가 변경되는 부분을 잡아주면 변동 사항을 빠르게 반영할 수 있다. 에러만 발생시키는 것이 이렇게 안정감을 줄 수 있다니.
