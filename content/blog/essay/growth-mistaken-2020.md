...
잔디를 채우기 위한 행위에 집중하지 말고 **작성하는 코드에 대한 고민**을 하거나 어떤 문제를 해결하는 것이 좀 더 의미있지 않을까. 일일커밋에 연연할 필요가 없다. 커밋으로 차오른 잔디는 코딩을 얼마나 했는지 척도일 뿐이다. 오히려 고민없이 작성한 코드는 잘못된 습관을 들이고 원하는 방향과 반대 방향으로 갈 수 있다.
...
블로그를 운영하는 것이야 말로 성장할 수 있는 확실한 방법이 아닌가? 이렇게 생각할 수 있다. 그러나 몸에 좋은 운동도 제대로 해야 효과가 있듯이 블로그도 의미있는 글을 써야 본인에게 도움이 된다.
...
JavaScript Weekly를 구독하고 있다면 새로운 proposal에는 무엇이 있는지, 어떤 기술이 새로 나왔는지 내가 사용하고 있는 라이브러리의 업데이트가 이루어졌는지 인지하고 있는 정도 도움이 된다. 강박적으로 모든 글을 읽을 필요는 없다.
