...
결론부터 말하자면 토스팀에 합류한지 8개월 정도 되었는데 정말 밀도 높은 성장을 했고 앞으로도 기대하고 있다. 내 경험은 턱없이 부족했고 깊이도 얕았다. 토스팀이라고 하면 조직 문화가 먼저 떠올랐지만 기술적인 부분도 배울 점이 정말 많았다. 조금 더 구체적으로 나열해보자면 다음과 같은 것들이 있을 것 같다.

- 디자인 시스템 (TDS)
- 모노레포를 통한 생산성 향상
- 효율적인 CI/CD 구축을 통한 생산성 향상
- 코드리뷰 시스템 구축을 통한 리뷰 활성화
- 재사용 가능한 함수 단위와 컴포넌트
- 좋은 코드에 대한 고민
- 챕터, 조직에 대한 비판적인 시각
...
> 세상은 넓고 괴수는 많다. 👉 [**토스팀은 채용중**](https://toss.im/career)
...
참석 인원의 이탈을 어떻게 최소화 할 수 있을지 고민을 많이 했다. 이 부분은 [토스 얼라이먼트 데이](https://blog.toss.im/2019/08/06/tossteam/culture/2019-2h-alignmentday/)에서 진행했던 Pop Quiz를 FEConf에 [적용](https://www.facebook.com/feconf.kr/posts/1009847012776478)하여 이탈을 최소화 할 수 있었다고 생각한다.
...
실제 NPM에 publish되는 소스 코드 뿐만 아니라 Document, Examples, CI/CD 등 정말 신경 쓸 것이 많았다. 단기적으로 어떤 결과물을 도출하기 위해 오버페이스로 달렸는데 생각보다 잘 따라주지 않아서 장기전으로 생각하고 천천히 다듬어보려고 한다.
