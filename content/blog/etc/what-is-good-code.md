...
위의 상황처럼 새로운 요구사항이 들어왔을 때, 커다란 함수 내부에서 응급처치하지 않아도 된다. 의존성을 기반으로 분리된 여러 함수들은 각자의 역할을 수행하고 있기 때문에 각자의 영역을 침범할 필요가 없다.
...
하드코딩이 불가피한 영역, 비즈니스 요구사항이 복잡하여 hacky한 코드를 작성해야 하는 경우 주석과 함께 코드를 격리시켜 관리할 수 있다.
