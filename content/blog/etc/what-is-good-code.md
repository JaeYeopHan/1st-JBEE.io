...
앞서 여러 가지를 이야기 했지만 결국, 처음부터 완벽한 코드를 작성하기엔 어렵고 끊임없이 개선해나갈 수 있는 코드를 작성하는 것이 중요하다. 결국 개선을 빠르게 할 수 있는 코드도 중요하지만 개선으로 인해 발생하는 문제를 조기에 감지할 수 있는 시스템이 필요하게 된다. 매번 수동으로 확인할 수 없으니 자연스럽게 '자동화 된 테스트'를 찾게 된다.
