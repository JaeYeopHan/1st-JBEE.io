...
독서의 계절을 맞아 책 추천 시리즈를 준비해봤다. 개인적으로 많은 도움을 받았거나 그동안 팀원에게 추천하고 함께 스터디 했던 책 5권을 선정했다. 단순 소개를 넘어 책을 언제 읽으면 좋을지, 어떻게 활용할지에 대한 내용 함께 담아보았다.

이런 글은 추천하는 사람이 누구인지 잘 살펴보고 받아들여야 한다. 개발을 어떻게 공부해왔는지, 최근에 주로 개발하는 영역은 무엇인지, 속해있는 팀의 규모는 어떤지 등 여러모로 개인의 경험과 배경, 주관이 많이 들어갈 수 밖에 없기 때문이다.
...
(벌써 재밌지 않은가) 방금 전에 작성한 코드라도 테스트 코드가 없다면 레거시 코드라는 것이다. 저마다 레거시 코드를 정의하지만 난 이 정의가 가장 마음에 들고 나에게 많은 변화를 준 문장이다. 테스트에 대한 이야기를 레거시 코드에서부터 시작하는 책이고 스크래치 리팩토링, 봉합 등 실질적으로 도움됐던 책이다.
...
프론트엔드 개발을 하고 있기에 덧붙여 보는 말인데, 간혹 이런 책들을 소개하면 '전 프론트엔드 개발자인데요?' 라는 분이 가끔 있다. 단연컨대 오늘 소개한 책들은 직군과는 상관없이 좋은 소프트웨어를 개발하는데 도움이 되는 책들이다. 책의 재료로 사용되는 언어가 JavaScript가 아니라고 해서 상관없는 책이라고 생각하지 말자. 오히려 책의 내용을 프론트엔드 개발이라는 상황으로 재해석해야 하기 때문에 더 적극적으로 이해하게 끔 한다.
...
- 이 명저를 이렇게 후순위로 소개하다니?라고 생각할 수도 있겠다. 이유가 있는 나만의 '설계'인데, 이 책에서 이야기하는 '설계'에 대한 중요성은 겪어봐야 납득이 되고 더 잘 이해할 수 있기 때문이다.
- 이유를 모른 채 계층을 나누면 계층을 나누는 데에만 집중하게 되고 코드가 엉뚱한 곳에 '배치'된다. 테스트 가능성에 대해 이야기하는데, 테스트를 작성하기 어려운 코드에 데여봐야 '테스트가 용이하려면 어떻게 해야 할까?' 고민을 할 것이고 그 때 이 책을 보면 도움이 될 것이다.

#### [테스트 주도 개발](https://www.aladin.co.kr/shop/wproduct.aspx?ItemId=37469717) 
- 이 책도 정말 유명한 책이다. 다만 덧붙이는 책에 불과한 이유는 타이밍 때문이다. 책의 내용이 좋고 유명한 것과 당장 나에게 도움되는 책인지는 다르기 때문이다.
...
- 이름이 귀여운 책이다. 그러나 내용은 귀엽지만은 않다. 함수형 프로그래밍에 대한 내용도 좋지만 기초적인 부분에 집중하고 있어 계층형 설계, 추상화 레벨에 대한 내용이 좋은 책이라고 소개한다.
- 이 역시 분량이 많아 두꺼운 책이지만 다 읽을 필요는 없는 책이다.

#### [이펙티브 소프트웨어 테스팅](https://www.aladin.co.kr/shop/wproduct.aspx?ItemId=311329128)
- Effective 시리즈 명성에 비해 부실하지만 몇 안 되는 테스트 책 중 하나이다. (그 시리즈인지는 모르겠지만) 좋은 테스트 코드에 대한 고민이 담겨있고 '테스트 코드의 유지 보수성을 위한 원칙' 챕터가 좋았다.
- 요약: https://tosspayments-dev.oopy.io/287802ad-84b9-42ad-9651-7c0901455274

'추천하는 책 5권 - 소프트스킬편'도 작성중이다. (업데이트 예정)
